...
class CodeAnalyzer:
    ...
    async def _extract_dependencies(self, content: str) -> List[str]:
        ...
        for line in import_lines:
            ...
            if 'import' in line or 'require' in line:
                import_parts = line.replace('import', '').replace('require', '') \
                              .replace('from', '').replace('"', "'").split("'")
    ...
    async def _analyze_with_regex(self, content: str, language: str) -> CodeAnalysisResult:
        ...
        complexity = {
            ...
            'cyclomatic_complexity': len([l for l in lines if any(kw in l for kw in ['if ', 'for ', 'while ', 'elif '])])
        }
