...
set "MEILI_PORT=18003"
set "MEILI_PATH=D:\meilisearch\bin\meilisearch.exe"

rem 从环境变量读取 MEILI_MASTER_KEY，未设置则使用默认值
if not defined MEILI_MASTER_KEY (
    echo [WARN] MEILI_MASTER_KEY not set, using default
    set "MEILI_MASTER_KEY=RDo25RtbmF8BSyLyOjBgpBOH8XZo1unrbu83Gz_rX4M"
)

set "MEILI_DB_PATH=D:/meilisearch/data/data.ms"
set "MAX_WAIT=30"
...
echo [CHECK] Meilisearch (port %MEILI_PORT%)...
curl -sf http://localhost:%MEILI_PORT%/health >nul 2>&1
if !errorlevel! equ 0 (
    echo   [OK] Already running
    set "MEILI_STATUS=running"
) else (
    echo   [--] Not running
    set "MEILI_STATUS=stopped"
)
...
:: ==================== 2. Start Meilisearch ====================
if "!MEILI_STATUS!"=="running" goto skip_meilisearch

echo [START] Launching Meilisearch...
start "Meilisearch" "%MEILI_PATH%" --http-addr 127.0.0.1:%MEILI_PORT% --master-key %MEILI_MASTER_KEY% --env development --db-path %MEILI_DB_PATH%
...
echo   Meilisearch:      http://localhost:%MEILI_PORT%/health
